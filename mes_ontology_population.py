...
def populate_from_csv(onto, csv_file, config):
    ...
    # Products
    product_df = df[['ProductID', 'ProductName', 'TargetRate_units_per_min', 
                     'StandardCost_per_unit', 'SalePrice_per_unit']].drop_duplicates()
    products = {}
    ...
    for _, row in product_df.iterrows():
        ...
        product.hasTargetRate = [float(row['TargetRate_units_per_min'])]
    ...
    for _, row in order_df.iterrows():
        order = get_or_create_individual(onto, onto.ProductionOrder, row['ProductionOrderID'])
...
def main():
    ...
    print(f"Creating MES Ontology: {config['ontology']['name']} v{config['ontology']['version']}")
    print("-" * 60)
    ...
    # Populate from CSV
    csv_file = "mes_data_with_kpis.csv"
    if os.path.exists(csv_file):
        populate_from_csv(onto, csv_file, config)
    else:
        print(f"Warning: {csv_file} not found. Please run mes_data_generation.py first.")
        return
    
    # Save ontology
    output_file = "mes_ontology_populated.owl"
