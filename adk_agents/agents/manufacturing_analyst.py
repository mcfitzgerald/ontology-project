...
class ManufacturingAnalystAgent:
    ...
    def _create_agent(self) -> genai.types.Agent:
    ...
    def _handle_tool_call(self, tool_call: Dict[str, Any], tool_context: types.ToolContext) -> Any:
        """Handle tool function calls from the agent."""
        function_name = tool_call.get("name")
        args = tool_call.get("args", {})
        
        if function_name == "execute_sparql":
            return execute_sparql(args.get("query"), tool_context)
            ...
            if hasattr(response, 'tool_calls'):
                for tool_call in response.tool_calls:
                    tool_result = self._handle_tool_call(
                        tool_call,
                        types.ToolContext()  # Create tool context
                    )
